...
{
	heating_enabled = 1;                                 //Enable heating
	HAL_GPIO_WritePin(TW_Valve, GPIO_PIN_RESET);
	heating_pwr = 0;                                    //Set heating power to xx%
	pump_speed = 80;                                     //Set pump speed to xx%
	set_led_status(LED_ID_WATER_OUT,LED_ON);
}
...
void pre_heat(WaterDispenser *Dispenser)
{
	...
	heating_enabled = 1;                                 //Enable heating
	HAL_GPIO_WritePin(TW_Valve, GPIO_PIN_SET);
	heating_pwr = 0;                                    //Set heating power to xx%
	pump_speed = 80;                                    //Set pump speed to xx%
}
...
{
	set_led_status(LED_ID_PRE_HEAT,LED_OFF);	
	set_led_status(LED_ID_DISINFECT,LED_BLINK);
	...
	heating_enabled = 1;                                 //Enable heating
	HAL_GPIO_WritePin(TW_Valve, GPIO_PIN_SET);
	heating_pwr = 80;                                    //Set heating power to xx%
	pump_speed = 60;                                    //Set pump speed to xx%
}
